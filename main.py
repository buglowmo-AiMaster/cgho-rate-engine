...
def setup(key: str = ""):
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_rate_quote ON rate_rules (rating_area_location, aoc, age_min, deductible_usd, coinsurance_pct)")
    ...
    conn.commit()
    conn.close()
    return {"status": "ok", "message": "Setup complete", "rates_loaded": len(rates), "next": "Test with /health"}
...
def quote(req: QuoteRequest, x_api_key: Optional[str] = Header(None)):
    ...
    if req.age < 70:
        ...
        age_clause = "AND r.age_min <= %s AND r.age_max >= %s"
        age_params = [req.age, req.age]
    plan_clause = "AND p.plan_tier = %s" if req.plan_tier else ""
    plan_params = [req.plan_tier] if req.plan_tier else []
    cur.execute("SELECT i.name AS insurer, p.name AS plan, p.plan_tier, r.aoc, r.deductible_usd, r.coinsurance_pct, r.oop_max_usd, r.hw_coverage, r.dv_coverage, r.ev_coverage, r.frequency, r.currency, r.base_premium FROM rate_rules r JOIN products p ON r.product_id = p.id JOIN insurers i ON r.insurer_id = i.id WHERE r.rating_area_location = %s AND r.aoc = %s AND r.deductible_usd = %s AND r.coinsurance_pct = %s AND r.frequency = %s " + age_clause + " " + plan_clause + " AND (r.expiry_date IS NULL OR r.expiry_date > CURRENT_DATE) ORDER BY r.base_premium ASC", [country["loc_id"], req.aoc, req.deductible_usd, req.coinsurance_pct, req.frequency] + age_params + plan_params)
    rows = cur.fetchall()
    conn.close()
    results = []
    for r in rows:
        ...
        if r["frequency"] == "Annual": monthly /= 12
        results.append({**dict(r), "base_premium": float(r["base_premium"]), "monthly_equivalent": round(monthly, 2), "annual_equivalent": round(monthly * 12, 2)})
    return {"age": req.age, "country": req.country_of_residence, "rating_zone": country["loc_zone"], "results_count": len(results), "results": results}
