...
def quote(req: QuoteRequest, x_api_key: Optional[str] = Header(None)):
    ...
    # Age band lookup — works for both exact ages (verified rates) and band-based ages (CSV data)
    cur.execute("""
    ...
    """, [country["loc_id"], req.aoc, req.deductible_usd, req.coinsurance_pct,
          req.frequency, req.age, req.age] + plan_params)
    ...
    # Sort by plan tier then premium
    tier_order = {"Silver": 1, "Gold": 2, "Platinum": 3}
    results.sort(key=lambda x: (tier_order.get(x["plan_tier"], 9), x["base_premium"]))
