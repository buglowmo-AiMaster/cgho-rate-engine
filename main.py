...
app = FastAPI(title="CGHO Rate Engine", version="2.0.0")
...
def get_rate(cur, loc_id, age, aoc, deductible, coinsurance, cover_type, plan_tier, cit_tier, hw=False, dv=False, ev=False):
    cur.execute("""
        SELECT r.base_premium FROM rate_rules r
        JOIN products p ON r.product_id = p.id
        WHERE r.rating_area_location = %s AND r.aoc = %s
        ...
        ORDER BY r.base_premium ASC LIMIT 1
    """, (loc_id, aoc, deductible, coinsurance, hw, dv, ev, age, age, plan_tier, cover_type, cit_tier))
    row = cur.fetchone()
    return float(row["base_premium"]) if row else None

@app.get("/")
def root():
    return {"api": "CGHO Rate Engine", "version": "2.0.0", "docs": "/docs", "health": "/health"}
...
def quote(req: QuoteRequest, x_api_key: Optional[str] = Header(None)):
    ...
    cur.execute("""
        ...
        FROM countries c JOIN rating_areas ra ON c.rating_area_location = ra.id
    ...
    for tier in tiers:
        ip_monthly = get_rate(cur, loc_id, req.age, req.aoc, req.ip_deductible_usd, req.ip_coinsurance_pct, "IP", tier, cit_tier)
        if ip_monthly is None: continue

        op_monthly = get_rate(cur, loc_id, req.age, req.aoc, req.op_deductible_usd, req.op_coinsurance_pct, "OP", tier, cit_tier) if req.include_op else None
        hw_monthly = get_rate(cur, loc_id, req.age, req.aoc, 0, 0, "IP", tier, cit_tier, hw=True) if req.include_hw else None
        dv_monthly = get_rate(cur, loc_id, req.age, req.aoc, 0, 0, "IP", tier, cit_tier, dv=True) if req.include_dv else None
        ev_monthly = get_rate(cur, loc_id, req.age, req.aoc, 0, 0, "IP", tier, cit_tier, ev=True) if req.include_ev else None
